...
def create_dataloaders(batchsize=28, img_size=28):
    ...
    data_augmentation = transforms.Compose([transforms.RandomResizedCrop(img_size),
                                            transforms.RandomRotation((30)),
                                            transforms.ToTensor()])
                                            #transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    # Load all data, create dataset
    dataset = ImageFolder(root="../data/Classification",
    ...
    train_set, test_set, val_set = torch.utils.data.random_split(dataset, [size_train, size_test, size_val])
