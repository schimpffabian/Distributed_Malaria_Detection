...
def compare_blob_detection(num_runs=15, threshold=4):
    ...
    log_time_avg = np.sum(log_time) / num_runs
    dog_time_avg = np.sum(dog_time) / num_runs
    doh_time_avg = np.sum(doh_time) / num_runs
    sbd_time_avg = np.sum(sbd_time) / num_runs
    ...
    print(
        ...
        % (log_time_avg, np.mean(log_acc[:, 0]), np.mean(log_acc[:, 2]))
    )

    print(
        "Difference of Gaussian: \t %2.5f s, TP: %2.2f,\t DC: %2.2f"
        % (dog_time_avg, np.mean(dog_acc[:, 0]), np.mean(dog_acc[:, 2]))
    )

    print(
        "Difference of Hessian:  \t %2.5f s, TP: %2.2f,\t DC: %2.2f"
        % (doh_time_avg, np.mean(doh_acc[:, 0]), np.mean(doh_acc[:, 2]))
    )

    print(
        "Simple Blob Detector:   \t %2.5f s, TP: %2.2f,\t DC: %2.2f"
        % (sbd_time_avg, np.mean(sbd_acc[:, 0]), np.mean(sbd_acc[:, 2]))
    )
    ...
    np.savetxt(
        ...
        save_list,
        delimiter=",",
        header="log_time, dog_time, doh_time, sbd_time",
    )
    np.savetxt(
